...
def save_eis_data(df_eis, filename, frame):
    # save eis data to csv
    df_eis.to_csv('C:/Users/Julian/PycharmProjects/EC_Analysis/CSV_data/'+filename+'.csv', mode='w', header=True,
                  index=False, sep='\t')

    # save eis data to excel
    path = 'C:/Users/Julian/PycharmProjects/EC_Analysis/EIS_data/eis_data_library.xlsx'
    ...
    #f = splrep(x_values, y_values, s=0)
    #f = InterpolatedUnivariateSpline(x_values, y_values, kind='linear')
    ...
    #hfr = root(f(x_values), 0)
    #print(interp1d.roots())
